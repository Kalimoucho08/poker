...
function npcThinkDelay(player) {
  ...
  // Plus rationnel = plus lent, plus agressif = plus rapide
  const baseSpeed = 800 + (1 - traits.rationality || 0.5) * 600 - (traits.aggression || 0.5) * 400;
  return Math.max(300, baseSpeed + (Math.random() - 0.5) * baseSpeed * 0.5);
}
...
function npcDecide(player, state) {
  let traits = { ...player.npcTraits };

  // V8: Adaptation au tournoi
  if (state._tournament && state._tournament.phase !== 'setup') {
    const bb = state.bigBlind;
    const stackBB = bb > 0 ? Math.floor(player.chips / bb) : 999;
    const tPhase = state._tournament.phase;
    traits = adaptTraitsToTournament(traits, stackBB, tPhase);
  }

  // V8: Appliquer le tilt
  if (typeof player._tiltLevel !== 'undefined' && player._tiltLevel > 0) {
    traits = applyTiltToTraits(traits, player._tiltLevel);
  }
  ...
  const catchphrase = player._blendCatchphrase || (
    (typeof NPC_TEMPLATES !== 'undefined' && NPC_TEMPLATES.find)
      ? (NPC_TEMPLATES.find(t => t.id === player.npcId) || {}).catchphrase
      : null
  );
  if (catchphrase && (action === 'raise' || (action === 'fold' && toCall > 0 && handStrength > 0.3))) {
    if (Math.random() < 0.15 && (typeof anonymousMode === 'undefined' || !anonymousMode)) {
      addLog(`  💬 ${player.name} : « ${catchphrase} »`);
    }
  }
